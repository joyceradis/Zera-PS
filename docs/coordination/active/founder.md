...
5. **Troponina — referência dependente do ensaio/kit:** não existe cutoff universal a ser hard-coded no produto. Interpretação/razão deve depender do valor medido, unidade e limite de referência do ensaio/laboratório. No Meridional, a Founder informa uso de troponina ultrassensível com referência local `0,0019`; esse número é **perfil/local**, não default universal. Se ensaio/unidade/referência não estiverem disponíveis, o Zera PS não deve inferir normalidade, positividade ou relação com o limite superior.
6. **Conduta/documento:** texto digitado pode permanecer natural na UI, mas saída final deve normalizar para UPPERCASE e prefixar linhas da conduta com `- `.
7. **Imagem/laudo:** microfunção `Formatar Imagem` deve gerar UPPERCASE e condensar o laudo colado em parágrafo único, removendo quebras de linha.
8. **Justificativa de alto custo:** evitar dropdowns engessados; campo livre para nome do exame + geração de justificativa contínua em UPPERCASE, usando QP/HDA, exame físico e hipóteses, com botão evidente de copiar.
9. **Produtividade:** resumo do plantão deve reconhecer snapshots/rascunhos correntes do Atendimento e contabilizar imediatamente, sem falso `ATENDIDOS: 0`/`SÉRIE INSUFICIENTE` quando já há atividade válida na sessão.
10. **Poluição visual:** aviso legal não deve competir com a tarefa clínica; mover/reduzir para rodapé ou apresentação discreta.
11. **Princípio transversal:** o teste da Founder é contínuo no preview publicado. Não existe uma segunda bateria escondida que ela ainda precise cumprir. O único gate final de domínio é o aceite explícito para piloto após as correções decorrentes.

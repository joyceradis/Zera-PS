...
const PROTECTED_BY = Object.freeze({
  ...
  'INV-CLIN-003': {
    ...
    protectors: [
      ...
      // Ponte de composição — a condição literal que o `gap` anterior exigia:
      // "contexto/progressive disclosure → coordenador real → estado/formulário entregue ao
      // document engine → documento final". Os vetores acima calculavam plan/visible/context e
      // depois renderizavam `emptyForm()`; o estado produzido pelo contexto nunca atravessava a
      // fronteira. Aqui o par (form, clinicalState) entregue ao document engine é PRODUZIDO
      // pelos escritores reais — coordenador de template, intake livre, composers de HDA e
      // funções de confirmação de proveniência — e o primeiro vetor é uma âncora
      // anti-trivialidade que reprova se o formulário voltar a chegar vazio.
